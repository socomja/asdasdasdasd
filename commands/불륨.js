...
module.exports = {
  ...
  run: async function (client, message, args) {
    ...
    if(parseInt(args[0]) > 300 ||(args[0]) < 0) return sendError('You can\'t set the volume more than 150. or lower than 0',message.channel).catch(err => console.log(err));
    serverQueue.volume = args[0]; 
    serverQueue.connection.dispatcher.setVolumeLogarithmic(args[0] / 100);
    const xd = new MessageEmbed()
    .setDescription(`현재 볼륨: **${args[0]/1}/300**`)
