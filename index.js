...
client.on('message', async message => {
    ...
    if (foundInText) {
        ...
        .setColor('Black')
        .setDescription(`뮤트가 억울하시면 <#793033448939454465>으로 이의제기 하시거나 관리자에게 이의제기를 부탁드립니다 도움을 드리지못해서 죄송합니다`);
...
client.on('message', async (message) => {
    ...
    let warn = '님이 욕하셨어요 조치를 취해주세요! <@&802365785816170506>'
...
client.on("message", msg => {
    ...
    if (msg.content === "!play") {
        ...
        let stream = ytdl("https://www.youtube.com/watch?v=6NUqPjE3ijw", {
            filter: "audioonly",
            opusEncoded: false,
            fmt: "mp3",
...
client.on('message', message => {
    if (message.content.startsWith(`${prefix}룰렛`)) { // 명령어는 (접두사)룰렛 <배팅할 코인> 입니다.
        ...
        if (bettingCoins > 100000) return message.reply(`한 번에 최대 100000코인만 배팅할 수 있어!`); // 최대 300코인 배팅 가능
        ...
        if (final_round > 0) { // 시작하자마자 죽지 않았다면
            setTimeout(() => {
                message.channel.send(`😱 ${name}, 너는 살아남았고, ${Math.floor(bettingCoins * 1.5)}코인을 받았어!`);
                nowReward = Math.floor(bettingCoins * 1.5); // 현재 보상은 배팅한 코인의 1.1배
                ...
                setTimeout(() => {
                    message.channel.send(`${name}, **계속** 을 입력하여 모든 보상을 걸고 다시 도전하여 ${Math.floor(bettingCoins * 1.8)}코인을 받을 수 있어!`);
                    ...
                    setTimeout(() => {
                        if (nowRound == 2) { // 현재 라운드가 2라운드이고,
                            if (final_round > 1) { // 2라운드에서도 살았다면,
                                ...
                                saveUser = {id: user.id, name: user.name, date: user.date, money: user.money + bettingCoins * 1.8 - bettingCoins * 1.8, xp: user.xp}; // saveUser 변수에 잔액을 변경
                                fs.writeFileSync(filePath, JSON.stringify(saveUser)); // data 폴더에 유저정보를 저장
                                setTimeout(() => {
                            ...
                            if (final_round > 2) {
                                ...
                                setTimeout(() => {
                                    message.channel.send(`${name}, **계속** 을 입력하여 ${nowReward}코인을 걸고 다시 도전하여 ${Math.floor(bettingCoins * 4.5)}코인을 받을 수 있어!`);
                                    continued = false;
                                    nowRound = nowRound + 1;
                                }, 2000);
                            }
                            ...
                            if (final_round > 3) {
                                message.channel.send(`😱 ${message.author.username}, 너는 살아남았고, ${Math.floor(bettingCoins * 4.5)}코인을 받았어!`);
                                nowReward = Math.floor(bettingCoins * 2.5);
                                saveUser = {id: user.id, name: user.name, date: user.date, money: user.money + bettingCoins * 4.5 - bettingCoins * 4.5, xp: user.xp};
                                fs.writeFileSync(filePath, JSON.stringify(saveUser));
                                setTimeout(() => {
                                    message.channel.send(`${name}, **계속** 을 입력하여 ${nowReward}코인을 걸고 다시 도전하여 ${Math.floor(bettingCoins * 15.5)}코인을 받을 수 있어!`);
                                    continued = false;
                                    nowRound = nowRound + 1;
                                }, 2000);
                            }
                            ...
                            if (final_round > 4) {
                                message.channel.send(`🥳 놀라워 ${message.author.username}, 너는 모든 라운드에서 살아남았고, ${Math.floor(bettingCoins * 15.5)}코인을 받았어!`);
...
client.on("message", (message) => {
   ...
   .setID("hh")
     ...
     buttonClient.send(null, { channel: message.channel.id, embed, buttons: [ [button1, button2, button4], [button3]]})
 }
})
...
buttonClient.on("hh", (inta) => {
	inta.message.reply("넌뭐야")
})
